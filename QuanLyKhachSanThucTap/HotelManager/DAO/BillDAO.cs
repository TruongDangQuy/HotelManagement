...
namespace HotelManager.DAO
{
    public class BillDAO
    {
        ...
        {
            string query = "USP_UpdateBill_Other1 @idBill , @discount";
            return DataProvider.Instance.ExecuteNoneQuery(query, new object[] { idBill, discount }) > 0;
        }
        internal bool UpdateOther1(int idBill, int paid)
        {
            string query = "update Bill" +
                "\r\nset Paid='"+paid+"'" +
                "\r\nwhere ID='"+idBill+"'";
            return DataProvider.Instance.ExecuteNoneQuery(query, new object[] { idBill, paid }) > 0;
        }
