...
namespace HotelManager
{
    public partial class fStaff : Form
    {
        ...
        //                                                    txbIDcard , comboBoxSex , txbPhoneNumber, txbAddress,txtPath});
        //    if (!isFill)
        //    {
        ...
        //            else
        //            {
        //                bool check = AccountDAO.Instance.UpdateAccount1(accountnow);
        //                if (check)
        //                {
        ...
        //}
        public void UpdateInfo(string username,string name, string address, int phonenumber, string idCard, DateTime dateOfBirth, string sex, string picture,DateTime startDay, int idStaffType)
        {
            AccountDAO.Instance.UpdateInfo1(username,name, address, phonenumber, idCard, dateOfBirth, sex, picture,startDay, idStaffType);
        }
        private void UpdateStaff()
        {
            bool isFill = fCustomer.CheckFillInText(new Control[] { txbUserName, comboBoxIdStaffType, txbName ,
                                                            txbIDcard , comboBoxSex , txbPhoneNumber, txbAddress,txtPath});
            if (!isFill)
            {
                MessageBox.Show("Không được để trống", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            else
            {
                ...
                try
                {
                    ...
                    {                        
                        UpdateInfo(txbUserName.Text, txbName.Text, txbAddress.Text, int.Parse(txbPhoneNumber.Text), txbIDcard.Text, datepickerDateOfBirth.Value, comboBoxSex.Text, txtPath.Text, datePickerStartDay.Value, int.Parse(comboBoxIdStaffType.Text));
        ...
        private void ChangeText(DataGridViewRow row)
        {
            ...
            else
            {
                ...
                txtPath.Text = row.Cells[Picture.Name].Value as string;
                string fname = txtPath.Text;
                string folder = @"D:\file";
                string pathstring = System.IO.Path.Combine(folder, fname);
                pictureBoxImage.Image = Image.FromFile(pathstring);
                ...
                comboBoxIdStaffType.SelectedIndex = (int)row.Cells[colIDStaffType.Name].Value - 1;
        ...
        private void btnChangImage_Click(object sender, EventArgs e)
        {
        ...
        private void saveStaff_FileOk(object sender, System.ComponentModel.CancelEventArgs e)
        {

        }

        private void bindingStaff_RefreshItems(object sender, EventArgs e)
        {
